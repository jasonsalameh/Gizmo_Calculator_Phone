...
namespace Calculator
{
    ...
    public class EvaluationEngine
    {
        ...
        protected string ParseNumberToken(TextParser parser)
        {
            ...
            while (Char.IsDigit(parser.Peek()) || parser.Peek() == '.' || (parser.Peek() == 'E' && (parser.Peek(1) == '+' || parser.Peek(1) == '-')))
            {
                if (parser.Peek() == '.')
                {
                    if (hasDecimal)
                ...
                if (parser.Peek() == 'E' && (parser.Peek(1) == '+' || parser.Peek(1) == '-'))
                    parser.MoveAhead();
        ...
        {
            int start = parser.Position;
            while (Char.IsLetterOrDigit(parser.Peek()) || parser.Peek() == '_' || parser.Peek() == '√' || parser.Peek() == '!')
                parser.MoveAhead();
        ...
        /// <returns></returns>
        protected string EvaluateFunction(TextParser parser, string name, int pos)
        {
        ...
        protected double ExecuteTokens(List<string> tokens)
        {
            ...
            foreach (string token in tokens)
            {
                ...
                else if (token == "P")
                {
                    ...
                    // n! / (n-k)!
                    double val = double.Parse(MainPage.factorial(tmp2)) / double.Parse(MainPage.factorial(tmp2 - tmp));
                ...
                else if (token == "C")
                {
                    ...
                    // n! / (k! * (n-k)!)
                    double val = double.Parse(MainPage.factorial(tmp2)) / (double.Parse(MainPage.factorial(tmp2 - tmp)) * double.Parse(MainPage.factorial(tmp)));
